...
3. Run these in parallel, where `<date>` is `YYYY-MM-DD`:
   - Merged: `gh search prs --author <login> --created ">=<date>" --merged --json repository,number,title,url,createdAt --limit 100`
   - In progress: `gh search prs --author <login> --created ">=<date>" --state open --json repository,number,title,url,createdAt,isDraft --limit 100`
   - Closed unmerged: `gh search prs --author <login> --created ">=<date>" --state closed --merged=false --json repository,number,title,url --limit 100`
